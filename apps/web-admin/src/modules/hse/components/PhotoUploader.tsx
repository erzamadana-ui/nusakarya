    ...
    <div>
      ...
      {paths.length === 0 ? (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
          {paths.map((p, i) => (
            <div key={p + i} className="relative group border border-ink-200 dark:border-ink-800 rounded-sm overflow-hidden">
              {previews[p] ? <img src={previews[p]} className="w-full h-20 object-cover" /> : <div className="w-full h-20 grid place-items-center bg-ink-50 dark:bg-surface-darker text-caption text-ink-400">Memuat…</div>}
              {!disabled && (
