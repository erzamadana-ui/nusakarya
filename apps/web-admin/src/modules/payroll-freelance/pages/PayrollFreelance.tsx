  ...
  return (
    <div>
            ...
            {dashLoading ? <TableSkeleton rows={4} /> : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyChart}>
                  ...
                  <Legend />
                  <Bar dataKey="bruto" name="Bruto" fill="#3AA3AA" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="netto" name="Netto" fill="#1B8A92" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>)}
          </div>
        </Card>
            ...
            {dashLoading ? <TableSkeleton rows={4} /> : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={compareChart}>
                  ...
                  <Tooltip formatter={(v: any) => rupiah(Number(v))} />
                  <Legend />
                  <Bar dataKey="fix_salary" name="Fix Salary (payroll_runs)" fill="#64748B" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="freelance" name="Freelance (freelance_payouts)" fill="#F5A524" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>)}
          </div>
        </Card>
          ...
          <div className="p-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topMitra} layout="vertical" margin={{ left: 24 }}>
                ...
                <Tooltip formatter={(v: any) => rupiah(Number(v))} />
                <Bar dataKey="netto" fill="#1B8A92" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>)}
      </Card>
      ...
      {filteredByTab.length > 0 && (
        <div className="flex flex-wrap gap-x-8 gap-y-1 px-4 py-3 mt-2 text-caption text-ink-500 border-t border-ink-200 dark:border-ink-800">
          <span>Total Bruto: <b className="text-ink-800 dark:text-ink-100">{rupiah(footerTotals.bruto)}</b></span>
          <span>Total DPP: <b className="text-ink-800 dark:text-ink-100">{rupiah(footerTotals.dpp)}</b></span>
          <span>Total Pajak: <b className="text-ink-800 dark:text-ink-100">{rupiah(footerTotals.pajak)}</b></span>
          <span>Total Netto: <b className="text-ink-800 dark:text-ink-100">{rupiah(footerTotals.netto)}</b></span>
        </div>
      )}
      ...
      <Modal open={payModal} onClose={() => setPayModal(false)} title="Tandai Dibayar"
        ...
        <div className="space-y-4">
          <p className="text-body text-ink-600 dark:text-ink-300">{selectedIds.length} payout akan ditandai dibayar.</p>
      ...
      <Modal open={calcOpen} onClose={() => setCalcOpen(false)} size="xl"
        ...
        {calcStep === 'pilih' && (
          <div className="space-y-4">
            ...
            <div className="bg-primary-50 dark:bg-primary-950 border border-primary-200 dark:border-primary-800 rounded-md p-3 text-caption text-ink-600 dark:text-ink-300">
                  ...
                  <thead className="bg-ink-50 dark:bg-surface-darker">
                    <tr>
                      {['Mitra', 'Pekerjaan', 'Bruto', 'Skema Pajak', 'DPP', 'Pajak', 'Potongan Lain', 'Netto', ''].map((h, i) => (
                        <th key={i} className="px-2 h-10 font-semibold text-caption uppercase text-ink-500 border-b border-ink-200 dark:border-ink-800 text-left whitespace-nowrap">{h}</th>))}
                    </tr>
                  </thead>
                  <tbody>
                    {preview.map((r, idx) => {
                      ...
                      return (
                        <tr key={r.employeeId} className={`border-b border-ink-100 dark:border-ink-800 align-top ${r.skip ? 'opacity-50' : ''}`}>
      ...
      <Drawer open={!!drawerRow} onClose={() => setDrawerRow(null)} title={drawerRow?.payout_no ?? ''} width="max-w-2xl"
        ...
        {drawerRow && <div id="self-billing-print">
          ...
          <div className="mt-5 grid grid-cols-2 gap-4 text-caption">
            <div>
              <p className="font-semibold text-ink-800 dark:text-ink-100 mb-1">{company?.name ?? '-'}</p>
            ...
            <div>
              <p className="font-semibold text-ink-800 dark:text-ink-100 mb-1">{drawerRow.employees?.full_name}</p>
          ...
          {drawerLines === null ? <TableSkeleton rows={4} /> : (
            <table className="w-full text-body">
              <thead><tr className="text-caption text-ink-500 border-b border-ink-200 dark:border-ink-800">
              ...
              <tbody>
                {drawerLines.map(l => (
                  <tr key={l.id} className="border-b border-ink-100 dark:border-ink-800">
          ...
          <div className="mt-5 max-w-xs ml-auto text-body">
            ...
            <div className="flex justify-between py-2 border-t border-ink-200 dark:border-ink-800 font-display font-bold text-[15px]"><span>Netto Diterima</span><span className="tabular">{rupiah(drawerRow.net_amount)}</span></div>
          </div>
