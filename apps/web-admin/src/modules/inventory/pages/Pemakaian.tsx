...
import {
 ...
 Button, Field, Input, Select, Badge, useToast, Plus, ConfirmDialog,
} from '@/components/ui'
...
export default function Pemakaian() {
 ...
 try {
 await remove('material_usages', confirmDelete.id)
 toast.push('Data pemakaian dihapus.', 'success')
 setConfirmDelete(null); load()
 } catch (e: any) {
 const msg = String(e?.message ?? '')
 toast.push(/row-level security|permission denied|RLS/i.test(msg)
 ? 'Gagal menghapus: Anda tidak memiliki hak Setujui pada modul Inventory. Hubungi admin untuk memberi hak akses.'
 : (msg || 'Gagal menghapus data pemakaian'), 'error')
 }
 }
 ...
 {can('INVENTORY', 'write') && <Button size="sm" variant="outline" onClick={() => openEdit(r)}>Koreksi</Button>}
 {can('INVENTORY', 'approve') && <Button size="sm" variant="ghost" className="text-red-600" onClick={() => setConfirmDelete(r)}>Hapus</Button>}
 </div>
 ),
 },
 ]
 ...
 title="Hapus Data Pemakaian" confirmLabel="Ya, Hapus"
 message={`Hapus catatan pemakaian item "${confirmDelete?.item?.name ?? ''}" tanggal ${confirmDelete ? tgl(confirmDelete.reported_at) : ''}? Tindakan ini tidak dapat dibatalkan.`} />
 </div>
 )
}
