...
import {
 ...
 Button, Field, Input, Select, Money, Textarea, Badge, useToast, Plus, ConfirmDialog,
} from '@/components/ui'
...
export default function PemeliharaanAset() {
 ...
 try {
 await remove('asset_maintenances', confirmDelete.id)
 toast.push('Riwayat pemeliharaan dihapus.', 'success')
 setConfirmDelete(null); load()
 } catch (e: any) {
 const msg = String(e?.message ?? '')
 toast.push(/row-level security|permission denied|RLS/i.test(msg)
 ? 'Gagal menghapus: Anda tidak memiliki hak Setujui pada modul Aset. Hubungi admin untuk memberi hak akses.'
 : (msg || 'Gagal menghapus data pemeliharaan'), 'error')
 }
 }
 ...
 {can('ASSET', 'write') && <Button size="sm" variant="outline" onClick={() => openEdit(r)}>Ubah</Button>}
 {can('ASSET', 'approve') && <Button size="sm" variant="ghost" className="text-red-600" onClick={() => setConfirmDelete(r)}>Hapus</Button>}
 </div>
 ),
 },
 ]
 ...
 title="Hapus Riwayat Pemeliharaan" confirmLabel="Ya, Hapus"
 message={`Hapus riwayat pemeliharaan aset "${confirmDelete?.asset_name ?? ''}" tanggal ${confirmDelete ? tgl(confirmDelete.maintenance_date) : ''}? Tindakan ini tidak dapat dibatalkan.`} />
 </div>
 )
}
