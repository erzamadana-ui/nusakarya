    ...
    <div>
      ...
      <FilterBar>
        <button onClick={() => setOnlyUnread(v => !v)}
          className={cx('h-8 px-3 rounded-full text-caption font-medium', onlyUnread ? 'bg-primary-500 text-white' : 'bg-ink-100 dark:bg-ink-800 text-ink-600 dark:text-ink-300')}>
          Belum Dibaca Saja
        </button>
      </FilterBar>
      ...
      {!loading && filtered.length === 0 ? (
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: 'title', header: 'Judul', render: r => <span className={cx(!r.is_read && 'font-semibold text-ink-900 dark:text-ink-50')}>{r.title}</span> },
