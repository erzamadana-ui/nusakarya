...
export default function InboxPersetujuan() {
 ...
 className={cx('h-8 px-3 rounded-full text-caption font-medium', moduleFilter === 'semua' ? 'bg-primary-500 text-white' : 'bg-ink-100 text-ink-600')}>
 Semua ({rows.length})
 </button>
 {approvableModules.filter(m => rows.some(r => r.module_code === m)).map(m => (
 <button key={m} onClick={() => setModuleFilter(m)}
 className={cx('h-8 px-3 rounded-full text-caption font-medium', moduleFilter === m ? 'bg-primary-500 text-white' : 'bg-ink-100 text-ink-600')}>
