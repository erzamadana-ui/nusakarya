          ...
          <div>
            ...
            <Section title="Foto Temuan">
              {galleryTemuan.length === 0 ? <EmptyState icon={<ImageIcon size={22} />} title="Belum ada foto temuan" /> : (
                <div className="grid grid-cols-3 gap-2">{galleryTemuan.map((u, i) => <a key={i} href={u} target="_blank" rel="noreferrer" className="block aspect-square rounded-sm overflow-hidden border border-ink-200 dark:border-ink-800"><img src={u} className="w-full h-full object-cover" /></a>)}</div>)}
            </Section>
            <Section title="Foto Perbaikan">
              {galleryPerbaikan.length === 0 ? <EmptyState icon={<CheckCircle2 size={22} />} title="Belum ada foto perbaikan" /> : (
                <div className="grid grid-cols-3 gap-2">{galleryPerbaikan.map((u, i) => <a key={i} href={u} target="_blank" rel="noreferrer" className="block aspect-square rounded-sm overflow-hidden border border-ink-200 dark:border-ink-800"><img src={u} className="w-full h-full object-cover" /></a>)}</div>)}
            </Section>
          </div>
        )}
      </Drawer>
