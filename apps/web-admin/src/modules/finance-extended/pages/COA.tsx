...
export default function COA() {
 ...
 return (
 <>
 <div className="flex items-center gap-2 px-3 py-2.5 border-b border-ink-100 hover:bg-primary-50/40">
 <div className="flex items-center gap-1.5 flex-1 min-w-0" style={{ paddingLeft: depth * 20 }}>
 {hasChildren ? (
 <button onClick={() => toggle(node.account_code)} className="p-0.5 rounded-xs text-ink-400 hover:bg-ink-100 shrink-0">
 {open ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
 </button>
 ) : <span className="w-5 shrink-0" />}
 <span className="font-mono text-caption text-ink-500 shrink-0 w-16">{node.account_code}</span>
 <span className={`text-body truncate ${!node.is_active ? 'text-ink-400 line-through' : 'text-ink-800'}`}>{node.account_name}</span>
 </div>
 ...
 <div className="flex items-center gap-2 px-3 py-2.5 border-b border-ink-200 bg-ink-50 text-caption font-semibold uppercase tracking-wide text-ink-500">
