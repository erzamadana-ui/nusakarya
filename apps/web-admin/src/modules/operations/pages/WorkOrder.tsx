            ...
            <Section title="Checklist Pekerjaan">
              {checklist.length === 0 ? <p className="text-caption text-ink-400">Tidak ada checklist untuk work order ini.</p> : (
                <div className="space-y-3">
                  {checklist.map(c => (
                    <div key={c.id} className="border border-ink-200 dark:border-ink-800 rounded-md p-3">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-body text-ink-800 dark:text-ink-100">{c.seq}. {c.question}{c.is_mandatory && <span className="text-red-500"> *</span>}</p>
                        {c.is_passed != null && <Badge tone={c.is_passed ? 'emerald' : 'red'}>{c.is_passed ? 'Sesuai' : 'Tidak Sesuai'}</Badge>}
                      </div>
                      <p className="text-body text-ink-600 dark:text-ink-300 mt-1">{c.answer_value || '-'}</p>
