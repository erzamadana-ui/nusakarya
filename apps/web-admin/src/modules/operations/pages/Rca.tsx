...
export default function Rca() {
  ...
  return (
    <div>
      ...
      <Card className="mb-4 bg-primary-50/50 dark:bg-primary-900/20 border-primary-200 dark:border-primary-800">
        <div className="p-4 text-body text-ink-700 dark:text-ink-200">
          <span className="font-semibold text-ink-900 dark:text-ink-50">Ringkasan: </span>{loading ? 'Memuat ringkasan…' : narasi}
        </div>
      </Card>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={perAspek}>
                  ...
                  <Bar dataKey="jumlah" radius={[4, 4, 0, 0]}>
                    {perAspek.map((r, i) => <Cell key={i} fill={ASPECT_COLORS[r.name] ?? '#1B8A92'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>)}
          </div>
