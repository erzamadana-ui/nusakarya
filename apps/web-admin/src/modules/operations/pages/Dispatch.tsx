            ...
            {WO_STATUSES.map(col => (
              <div key={col.value} className="flex-1 min-w-[220px]">
                <div className="flex items-center justify-between px-1 mb-2">
                  <h4 className="font-display font-semibold text-body text-ink-700 dark:text-ink-200">{col.label}</h4>
                  <Badge tone={col.tone}>{byStatus(col.value).length}</Badge>
                </div>
                <div className="space-y-2 bg-ink-50 dark:bg-surface-darker rounded-md p-2 min-h-[200px]">
                  {loading ? <TableSkeleton rows={3} /> : byStatus(col.value).length === 0 ? (
                    ...
                    <Card key={wo.id} className="p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-body text-ink-900 dark:text-ink-50">{wo.wo_no}</span>
                        <Badge tone="teal">{WO_TYPES.find(t => t.value === wo.wo_type)?.label ?? wo.wo_type}</Badge>
                      </div>
                      <p className="text-body text-ink-700 dark:text-ink-200">{wo.customer_name || '-'}</p>
              ...
              <ul className="divide-y divide-ink-100 dark:divide-ink-800">
                {technicians.map(t => (
                  <li key={t.id} className="flex items-center justify-between px-2 py-2.5">
                    <div>
                      <p className="text-body text-ink-800 dark:text-ink-100">{t.full_name}</p>
