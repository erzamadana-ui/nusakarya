...
export default function Tiket() {
 ...
 {selected.description && <p className="mt-4 text-body text-ink-600">{selected.description}</p>}
 </Section>
 )}
 ...
 <span className="inline-flex items-center h-9 px-3 rounded-sm border border-ink-200 text-body cursor-pointer hover:bg-ink-50">Unggah Lampiran</span>
 ...
 {attachments.map(a => (
 <a key={a.id} href={attUrls[a.id] || '#'} target="_blank" rel="noreferrer" className="block border border-ink-200 rounded-md p-2 hover:shadow-e1">
 ...
 : <div className="w-full h-24 grid place-items-center bg-ink-50 rounded-xs mb-1.5 text-ink-400 text-caption">Berkas</div>}
 <p className="text-caption truncate text-ink-700">{a.file_name}</p>
 </a>))}
 </div>)}
 </Section>
 )}
