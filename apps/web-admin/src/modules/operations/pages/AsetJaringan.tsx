...
export default function AsetJaringan() {
 ...
 { label: 'Koordinat', value: <>{selected.lat && selected.lng ? `${selected.lat}, ${selected.lng}` : '-'}{peta && <a href={peta} target="_blank" rel="noreferrer" className="block text-primary-600 mt-0.5">Lihat di Google Maps</a>}</> },
 ]} />
 </Section>
 ...
 <ul className="space-y-1">{anak(selected.id).map(a => <li key={a.id} className="text-body text-ink-700">• {a.element_type} · {a.code} — {a.name}</li>)}</ul>)}
 </Section>
 ...
 <span className="text-ink-700">{p.plan_no} · {p.plan_name}</span>
