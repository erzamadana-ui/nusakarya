                  ...
                  <thead className="bg-ink-50 dark:bg-surface-darker">
                    <tr>
                      {['Karyawan', 'Bruto', 'BPJS Perusahaan', 'BPJS Pekerja', 'Lembur', 'Insentif Produktivitas', 'PPh 21', 'Netto', ''].map((h, i) => (
                        <th key={i} className={`px-3 h-11 font-semibold text-caption uppercase text-ink-500 border-b border-ink-200 dark:border-ink-800 ${i > 0 && i < 8 ? 'text-right' : 'text-left'}`}>{h}</th>))}
                    </tr>
                  </thead>
                  <tbody>
                    {runs.map(r => (
                      <tr key={r.id} className="border-b border-ink-100 dark:border-ink-800 hover:bg-primary-50/40 dark:hover:bg-ink-800/50 cursor-pointer" onClick={() => openSlip(r)}>
                        <td className="px-3 py-2.5 text-ink-800 dark:text-ink-100 font-medium">{r.employees?.full_name}</td>
          ...
          <div className="mt-5">
            {slipLines === null ? <TableSkeleton rows={4} /> : (
              <table className="w-full text-body">
                <tbody>
                  {slipLines.filter(l => l.component_type === 'earning').map(l => <tr key={l.id}><td className="py-1 text-ink-600 dark:text-ink-300">{l.component_name}</td><td className="py-1 text-right tabular">{rupiah(l.amount)}</td></tr>)}
                  <tr className="border-t border-ink-200 dark:border-ink-800"><td className="py-1.5 font-semibold">Total Bruto</td><td className="py-1.5 text-right tabular font-semibold">{rupiah(slip.gross)}</td></tr>
                  {slipLines.filter(l => l.component_type === 'deduction').map(l => <tr key={l.id}><td className="py-1 text-ink-600 dark:text-ink-300">{l.component_name}</td><td className="py-1 text-right tabular text-red-600">-{rupiah(l.amount)}</td></tr>)}
                  <tr className="border-t border-ink-200 dark:border-ink-800"><td className="py-2 font-display font-bold text-[15px]">Gaji Diterima (Netto)</td><td className="py-2 text-right tabular font-display font-bold text-[15px]">{rupiah(slip.net_pay)}</td></tr>
                </tbody>
              </table>)}
          </div>
