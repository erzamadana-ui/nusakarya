...
import {
 PageHeader, DataTable, Badge, Button, Modal, Drawer, ConfirmDialog, Field, Select, Input, Money, Checkbox, useToast, Plus, EmptyState, TableSkeleton,
} from '@/components/ui'
...
export default function KomponenGaji() {
 ...
 const [delAssignId, setDelAssignId] = useState<string | null>(null)
 ...
 if (Number(form.default_amount) < 0) { toast.push('Nominal default tidak boleh negatif', 'error'); return }
 setSaving(true)
 try {
 ...
 const msg = /row-level security|permission denied/i.test(e.message ?? '') ? 'Gagal menyimpan komponen gaji — hak akses Anda pada modul PAYROLL tidak mengizinkan penulisan (perlu izin write).' : (e.message ?? 'Gagal menyimpan komponen gaji')
 ...
 if (assignForm.end_date && assignForm.end_date < assignForm.effective_date) { toast.push('Tanggal berakhir tidak boleh mendahului tanggal berlaku', 'error'); return }
 if (Number(assignForm.amount) < 0) { toast.push('Nominal tidak boleh negatif', 'error'); return }
 ...
 const msg = /row-level security|permission denied/i.test(e.message ?? '') ? 'Gagal menetapkan komponen gaji — hak akses Anda pada modul PAYROLL tidak mengizinkan penulisan (perlu izin write).' : (e.message ?? 'Gagal menetapkan komponen gaji')
 toast.push(msg, 'error')
 }
 ...
 const msg = /row-level security|permission denied/i.test(e.message ?? '') ? 'Gagal menghapus penetapan — hak akses Anda pada modul PAYROLL tidak mengizinkan penghapusan (perlu izin write).' : (e.message ?? 'Gagal menghapus penetapan')
 toast.push(msg, 'error')
 }
 finally { setDelAssignId(null) }
 }
 ...
 {empSel && <>
 {can('PAYROLL', 'write') && (
 <div className="mt-4 mb-5 p-3 rounded-md border border-ink-200 grid sm:grid-cols-4 gap-3 items-end">
 ...
 {assignments.map(a => (
 <div key={a.id} className="flex items-center justify-between px-3 py-2 rounded-sm border border-ink-200">
 <div>
 <p className="font-medium text-ink-800">{a.salary_components?.name}</p>
 <p className="text-caption text-ink-400">{rupiah(a.amount)} · sejak {tgl(a.effective_date)}{a.end_date ? ` s.d. ${tgl(a.end_date)}` : ''}</p>
 </div>
 {can('PAYROLL', 'write') && <Button size="sm" variant="ghost" className="text-red-600" onClick={() => setDelAssignId(a.id)}>Hapus</Button>}
 </div>))}
 </div>)}
 </>}
 </Drawer>

 <ConfirmDialog open={!!delAssignId} onClose={() => setDelAssignId(null)} onConfirm={() => removeAssignment(delAssignId!)}
 title="Hapus Penetapan Komponen" message="Penetapan komponen gaji ini akan dihapus dari karyawan terpilih." danger confirmLabel="Ya, hapus" />
 </div>
 )
}
