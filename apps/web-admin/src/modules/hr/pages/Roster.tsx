...
import {
  PageHeader, Card, Button, Modal, Field, Select, Input, Badge, useToast, EmptyState, TableSkeleton,
} from '@/components/ui'
...
export default function Roster() {
  ...
  async function saveCell() {
    ...
    } catch (e: any) { toast.push(e.message ?? 'Gagal menyimpan jadwal', 'error') }
  ...
  async function deleteShift(id: string) {
    try { await remove('shifts', id); toast.push('Shift dihapus'); load() } catch (e: any) { toast.push(e.message ?? 'Gagal menghapus shift', 'error') }
  }
  ...
  return (
    <div>
      ...
      <Card className="mb-5 p-3 flex items-center justify-between flex-wrap gap-2">
        ...
        <span className="text-body font-medium text-ink-700 dark:text-ink-200">{tgl(iso(monday))} – {tgl(iso(week[6]))}</span>
      ...
      <Card className="overflow-auto">
        {loading ? <TableSkeleton rows={6} /> : employees.length === 0 ? <EmptyState title="Belum ada karyawan aktif" /> : (
          <table className="w-full text-body border-separate border-spacing-0 min-w-[760px]">
            <thead className="bg-ink-50 dark:bg-surface-darker sticky top-0">
              <tr>
                <th className="px-3 h-11 text-left font-semibold text-caption uppercase text-ink-500 border-b border-ink-200 dark:border-ink-800 sticky left-0 bg-ink-50 dark:bg-surface-darker">Karyawan</th>
                {week.map((d, i) => <th key={i} className="px-2 h-11 text-center font-semibold text-caption uppercase text-ink-500 border-b border-ink-200 dark:border-ink-800 whitespace-nowrap">{DAY_LABEL[i]}<br /><span className="font-normal normal-case">{d.getDate()}/{d.getMonth() + 1}</span></th>)}
              </tr>
            </thead>
            <tbody>
              {employees.map(emp => (
                <tr key={emp.id} className="border-b border-ink-100 dark:border-ink-800">
                  <td className="px-3 py-2 sticky left-0 bg-white dark:bg-surface-dark text-ink-800 dark:text-ink-100 font-medium whitespace-nowrap">{emp.full_name}</td>
                  {week.map((d, i) => {
                    const key = `${emp.id}_${iso(d)}`; const r = rosterMap[key]; const sh = shifts.find(s => s.id === r?.shift_id)
                    return <td key={i} className="px-1 py-1.5 text-center cursor-pointer hover:bg-primary-50/60 dark:hover:bg-ink-800/60" onClick={() => openCell(emp, d)}>
      ...
      <Modal open={!!cell} onClose={() => setCell(null)} title="Tetapkan Shift" size="sm"
        ...
        {cell && <div className="space-y-3">
          <p className="text-body text-ink-600 dark:text-ink-300">{cell.emp.full_name} · {tgl(cell.date)}</p>
      ...
      <Modal open={shiftModal} onClose={() => setShiftModal(false)} title="Kelola Master Shift" size="md"
        ...
        <div className="space-y-1.5">
          {shifts.map(s => (
            <div key={s.id} className="flex items-center justify-between px-3 py-2 rounded-sm border border-ink-200 dark:border-ink-800">
              <div><span className="font-medium text-ink-800 dark:text-ink-100">{s.code}</span> <span className="text-ink-500">— {s.name}</span> <span className="text-caption text-ink-400">({s.start_time?.slice(0, 5)}–{s.end_time?.slice(0, 5)})</span></div>
              <div className="flex items-center gap-1">
                <Button size="sm" variant="ghost" onClick={() => openEditShift(s)}>Ubah</Button>
                <Button size="sm" variant="ghost" onClick={() => deleteShift(s.id)}><X size={14} /></Button>
              </div>
            </div>))}
