      ...
      <Card className="mb-5">
        ...
        {loading ? <TableSkeleton rows={2} /> : populer.length === 0 ? (
          ...
          <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {populer.map(a => (
              <button key={a.id} onClick={() => bukaArtikel(a)} className="text-left p-3 rounded-md border border-ink-200 dark:border-ink-800 hover:shadow-e1 transition-shadow">
                <p className="text-body font-medium text-ink-800 dark:text-ink-100 line-clamp-2">{a.title}</p>
          ...
          <div className="max-h-[calc(100vh-460px)] min-h-[300px] overflow-y-auto">
            {loading ? <TableSkeleton rows={6} /> : filtered.length === 0 ? (
              ...
              <button key={a.id} onClick={() => bukaArtikel(a)}
                className={cx('block w-full text-left px-4 py-3 border-b border-ink-100 dark:border-ink-800 hover:bg-primary-50/50 dark:hover:bg-ink-800/60',
                  selected?.id === a.id && 'bg-primary-50 dark:bg-ink-800')}>
                <div className="flex items-start justify-between gap-2">
                  <p className="text-body font-medium text-ink-800 dark:text-ink-100 line-clamp-1">{a.title}</p>
            ...
            <div>
              ...
              <div className="p-5 space-y-5">
                ...
                <Section title="Gejala">
                  <p className="text-body text-ink-700 dark:text-ink-200 whitespace-pre-line">{selected.symptom || '-'}</p>
                </Section>
                <Section title="Akar Masalah">
                  <p className="text-body text-ink-700 dark:text-ink-200 whitespace-pre-line">{selected.root_cause || '-'}</p>
                </Section>
                <Section title="Langkah Penyelesaian">
                  <p className="text-body text-ink-700 dark:text-ink-200 whitespace-pre-line">{selected.resolution_steps || '-'}</p>
                </Section>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selected.attachments.map((att: any, i: number) => (
                        <a key={i} href={attUrls[att.path] || '#'} target="_blank" rel="noreferrer"
                          className="text-caption px-3 py-1.5 rounded-sm border border-ink-200 dark:border-ink-700 text-primary-600 dark:text-primary-300 hover:bg-ink-50 dark:hover:bg-ink-800">
          ...
          <div className="sm:col-span-2">
            <Field label="Lampiran">
              <label className="inline-block">
                <span className="inline-flex items-center h-9 px-3 rounded-sm border border-ink-200 dark:border-ink-700 text-body cursor-pointer hover:bg-ink-50 dark:hover:bg-ink-800">
              ...
              {formAttachments.length > 0 && (
                <ul className="mt-2 space-y-1">
                  {formAttachments.map((att, i) => (
                    <li key={i} className="flex items-center justify-between text-caption text-ink-600 dark:text-ink-300">
