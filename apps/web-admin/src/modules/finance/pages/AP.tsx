  ...
  return (
    <div>
      ...
      <DataTable
        ...
        columns={[
          { key: 'inv_no', header: 'No Invoice', render: (r) => <div><div className="font-medium text-ink-900 dark:text-ink-50">{r.inv_no}</div>{r.vendor_invoice_no && <div className="text-caption text-ink-400">{r.vendor_invoice_no}</div>}</div> },
                ...
                <div className="space-y-2">
                  {chk.items.map(it => (
                    <div key={it.key} className="flex items-center gap-2 text-body">
                      ...
                      <span className={it.ok ? 'text-ink-700 dark:text-ink-200' : 'text-red-600'}>{it.label}</span>
                    </div>
                  ))}
                </div>
      ...
      <Modal open={batchOpen} onClose={() => setBatchOpen(false)} title="Buat Batch Pembayaran" size="lg"
        ...
        <div className="border border-ink-200 dark:border-ink-800 rounded-md divide-y divide-ink-100 dark:divide-ink-800 max-h-52 overflow-y-auto">
