              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={apAgingChart}>
                  ...
                  <Bar dataKey="amount" name="Sisa AP" radius={[3, 3, 0, 0]}>
                    {apAgingChart.map((d, i) => <Cell key={i} fill={agingTone(d.bucket) === 'red' ? '#E11D48' : agingTone(d.bucket) === 'orange' ? '#F59E0B' : '#F5A524'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </Card>
                ...
                <ResponsiveContainer width="100%" height={Math.max(220, marginProyek.length * 34)}>
                  <BarChart data={marginProyek} layout="vertical" margin={{ left: 24 }}>
                    ...
                    <Bar dataKey="marginPct" name="marginPct" radius={[0, 3, 3, 0]}>
                      {marginProyek.map((d, i) => <Cell key={i} fill={d.marginPct < 10 ? '#E11D48' : CHART_COLORS.primary} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
          ...
          <Section title="Kepatuhan SLA Pembayaran Mitra">
            <Card className="p-4">
              <div className="grid sm:grid-cols-3 gap-3 mb-4">
                <div className="p-3 rounded-md bg-ink-50 dark:bg-surface-darker">
                  <p className="text-caption text-ink-500">Rata-rata Hari Bayar Aktual</p>
                  <p className="font-display text-[22px] font-bold text-ink-900 dark:text-ink-50 mt-1">{sla.avgActual != null ? `${sla.avgActual.toFixed(1)} hari` : '-'}</p>
                </div>
                <div className="p-3 rounded-md bg-ink-50 dark:bg-surface-darker">
                  <p className="text-caption text-ink-500">Rata-rata Target SLA</p>
                  <p className="font-display text-[22px] font-bold text-ink-900 dark:text-ink-50 mt-1">{sla.avgTarget != null ? `${sla.avgTarget.toFixed(1)} hari` : '-'}</p>
                </div>
                <div className="p-3 rounded-md bg-ink-50 dark:bg-surface-darker">
              ...
              <p className="text-caption font-medium text-ink-600 dark:text-ink-300 mb-2">Invoice yang akan / sudah melewati SLA pembayaran</p>
              <DataTable
                searchable={false}
                columns={[
