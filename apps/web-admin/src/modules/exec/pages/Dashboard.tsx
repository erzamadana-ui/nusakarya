...
const COLORS = ['#1B8A92', '#F5A524', '#3AA3AA', '#F59E0B', '#64748B', '#E11D48', '#16A34A']
              ...
              {loading ? <Skeleton className="h-full w-full" /> : (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={finance} margin={{ left: 4, right: 8, top: 4, bottom: 0 }}>
                    ...
                    <Bar dataKey="biaya" name="Biaya" fill="#F5A524" radius={[3, 3, 0, 0]} />
                    <Line type="monotone" dataKey="pendapatan" name="Pendapatan" stroke="#1B8A92" strokeWidth={2.5} dot={false} />
                  </ComposedChart>
                </ResponsiveContainer>
              )}
            </div>
              ...
              {loading ? <Skeleton className="h-full w-full" /> : projects.length === 0 ? <EmptyState title="Tidak ada proyek aktif" /> : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={projects} layout="vertical" margin={{ left: 8, right: 24 }}>
                    ...
                    <Bar dataKey="progress_percent" name="Progres" fill="#1B8A92" radius={[0, 3, 3, 0]}>
              ...
              {loading ? <Skeleton className="h-full w-full" /> : productivity.length === 0 ? <EmptyState title="Belum ada data produktivitas" /> : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={productivity} layout="vertical" margin={{ left: 8, right: 24 }}>
                    ...
                    <Bar dataKey="total_points" name="Poin" fill="#3AA3AA" radius={[0, 3, 3, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          <ul className="divide-y divide-ink-100 dark:divide-ink-800">
            {pending.map(item => (
              <li key={`${item.type}-${item.id}`}>
                <Link to={item.path} className="flex items-center justify-between gap-3 px-5 py-3 hover:bg-primary-50/50 dark:hover:bg-ink-800/60 transition-colors">
                  <div className="min-w-0">
                    <p className="text-caption text-ink-400">{item.type}</p>
                    <p className="text-body text-ink-800 dark:text-ink-100 truncate">{item.title}</p>
                  </div>
