...
export default function Perjalanan() {
 ...
 if (form.end_date < form.start_date) { toast.push('Tanggal kembali tidak boleh mendahului tanggal berangkat', 'error'); return }
 if (Number(form.daily_allowance) < 0 || Number(form.total_advance) < 0) { toast.push('Nominal uang saku dan uang muka tidak boleh negatif', 'error'); return }
 setSaving(true)
 try {
 ...
 if (Number(expForm.amount) < 0) { toast.push('Jumlah biaya tidak boleh negatif', 'error'); return }
 setExpSaving(true)
 try {
