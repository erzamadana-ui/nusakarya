      ...
      <Drawer open={!!detail} onClose={() => setDetail(null)} title={detail?.contract_no} width="max-w-2xl"
        ...
        {detail && (
          <>
            ...
            {detail.note && <p className="text-body text-ink-600 dark:text-ink-300 mt-3">{detail.note}</p>}
            ...
            {posInPeriod.length === 0 ? <EmptyState title="Belum ada PO pada periode ini" /> : (
              <div className="space-y-1.5 mb-3">
                {posInPeriod.map((p: any) => (
                  <div key={p.id} className="flex items-center justify-between text-body px-3 py-2 rounded-sm bg-ink-50 dark:bg-surface-darker">
                    <span>{p.po_no} <span className="text-caption text-ink-400">· {tgl(p.po_date)}</span></span>
                    <span className="tabular">{rupiah(p.total)}</span>
                  </div>))}
              </div>
            )}
            <div className="flex items-center justify-between text-body-l font-semibold border-t border-ink-200 dark:border-ink-800 pt-2">
