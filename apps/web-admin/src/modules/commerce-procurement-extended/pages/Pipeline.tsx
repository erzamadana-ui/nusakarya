            ...
            {loading ? <TableSkeleton rows={4} /> : (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={trendData} margin={{ left: 4, right: 8, top: 8, bottom: 0 }}>
                  ...
                  <Bar dataKey="Menang" fill="#16A34A" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="Kalah" fill="#E11D48" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>)}
          </div>
        </Card>
      ...
      {loading ? <Card><TableSkeleton rows={8} /></Card> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          {PIPELINE_STAGES.map(stage => {
            ...
            return (
              <div key={stage.value} className="bg-ink-50 dark:bg-surface-darker rounded-md border border-ink-200 dark:border-ink-800 flex flex-col min-h-[200px]">
                <div className="px-3 py-2.5 border-b border-ink-200 dark:border-ink-800">
                  <div className="flex items-center justify-between">
                    <span className="font-display font-semibold text-[13px] text-ink-800 dark:text-ink-100">{stage.label}</span>
                  ...
                  {list_.map(row => {
                    ...
                    return (
                      <Card key={row.id} onClick={() => openDetail(row)} className="p-2.5 cursor-pointer hover:shadow-e2 transition-shadow">
                        <p className="text-body font-medium text-ink-900 dark:text-ink-50 leading-snug line-clamp-2">{row.title}</p>
                        <p className="text-caption text-ink-500 mt-0.5">{custMap[row.customer_id] ?? '-'}</p>
                        <div className="flex items-center justify-between mt-1.5">
                          <span className="text-body font-semibold tabular text-ink-800 dark:text-ink-100">{rupiah(row.estimated_value, true)}</span>
          ...
          <>
            ...
            {detail.stage === 'kalah' && detail.lost_reason && (
              <Card className="mb-4 p-3 bg-red-50/60 dark:bg-red-950/30 border-red-200 dark:border-red-900">
                ...
                <p className="text-body text-ink-700 dark:text-ink-200">{detail.lost_reason}</p>
              </Card>)}
