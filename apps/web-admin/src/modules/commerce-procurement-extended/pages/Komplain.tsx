    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <button key={i} type="button" disabled={!onChange}
          ...
          <Star size={size} className={i <= (value ?? 0) ? 'fill-amber-400 text-amber-400' : 'text-ink-300 dark:text-ink-700'} />
        </button>
      ))}
    </div>
  )
          ...
          <div className="p-4 space-y-2.5">
            {loading ? <TableSkeleton rows={4} /> : topCustomers.length === 0 ? <EmptyState title="Belum ada komplain" /> : topCustomers.map(c => (
              <div key={c.id} className="flex items-center justify-between gap-2">
                <span className="text-body text-ink-700 dark:text-ink-200 truncate">{c.name}</span>
