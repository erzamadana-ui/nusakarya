...
export default function Spk() {
 ...
 if (form.start_date && form.end_date && form.end_date < form.start_date) { toast.push('Tanggal selesai tidak boleh mendahului tanggal mulai', 'error'); return }
 if (Number(form.spk_value) < 0) { toast.push('Nilai SPK tidak boleh negatif', 'error'); return }
 setSaving(true)
 try {
 const payload = {
