  ...
  return (
    <div>
      ...
      <Drawer open={!!detail} onClose={() => setDetail(null)} title="Detail Perubahan"
        footer={<button onClick={() => setDetail(null)} className="h-10 px-4 rounded-sm text-body font-medium border border-ink-200 dark:border-ink-700">Tutup</button>}>
        {detail && (
          <div className="space-y-4">
            <dl className="grid grid-cols-2 gap-3 text-body">
              <div><dt className="text-caption text-ink-400">Waktu</dt><dd className="text-ink-800 dark:text-ink-100">{tglJam(detail.created_at)}</dd></div>
              <div><dt className="text-caption text-ink-400">Pengguna</dt><dd className="text-ink-800 dark:text-ink-100">{userNames[detail.user_id ?? ''] ?? 'Sistem'}</dd></div>
              <div><dt className="text-caption text-ink-400">Aksi</dt><dd><Badge>{detail.action}</Badge></dd></div>
              <div><dt className="text-caption text-ink-400">Entitas</dt><dd className="text-ink-800 dark:text-ink-100">{detail.entity_type} {detail.entity_id ? `(${detail.entity_id})` : ''}</dd></div>
            </dl>
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <p className="text-caption font-medium text-ink-500 uppercase tracking-wide mb-1.5">Sebelum</p>
                <pre className="text-caption whitespace-pre-wrap break-all bg-ink-50 dark:bg-ink-900 border border-ink-200 dark:border-ink-800 rounded-sm p-3 max-h-80 overflow-auto">
