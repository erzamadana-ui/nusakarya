      ...
      {loading ? <Skeleton className="h-96 w-full rounded-md" /> : (
        <Card className="overflow-auto">
          <table className="min-w-full text-body border-separate border-spacing-0">
            <thead>
              <tr>
                <th className="sticky left-0 z-20 bg-ink-50 dark:bg-surface-darker px-4 h-11 text-left font-semibold text-caption uppercase tracking-wide text-ink-500 border-b border-r border-ink-200 dark:border-ink-800 whitespace-nowrap">Jabatan</th>
                {modules.map(m => (
                  <th key={m.code} className="bg-ink-50 dark:bg-surface-darker px-3 h-11 text-center font-semibold text-caption uppercase tracking-wide text-ink-500 border-b border-ink-200 dark:border-ink-800 whitespace-nowrap min-w-[132px]">{m.name}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {roles.map(role => (
                <tr key={role} className="border-b border-ink-100 dark:border-ink-800">
                  <td className="sticky left-0 z-10 bg-white dark:bg-surface-dark px-4 py-2.5 font-medium text-ink-800 dark:text-ink-100 border-r border-ink-200 dark:border-ink-800 whitespace-nowrap">{ROLE_LABEL[role]}</td>
                  {modules.map(m => {
