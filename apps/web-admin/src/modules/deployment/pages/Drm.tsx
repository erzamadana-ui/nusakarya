        ...
        {openAiTasks.length === 0 ? <EmptyState title="Tidak ada action item terbuka" /> : (
          <div className="overflow-auto max-h-64">
            <table className="w-full text-body">
              <thead className="bg-ink-50 dark:bg-surface-darker sticky top-0"><tr>
          ...
          <div>
            ...
            <Section title="Peserta">
              {(drawerRow.participants ?? []).length === 0 ? <p className="text-caption text-ink-400">Tidak ada data peserta.</p> : (
                <ul className="text-body text-ink-700 dark:text-ink-200 space-y-1">{(drawerRow.participants ?? []).map((p: any, i: number) => <li key={i}>{p.name} — {p.instansi}</li>)}</ul>)}
            </Section>
            {drawerRow.agenda && <Section title="Agenda"><p className="text-body whitespace-pre-line text-ink-700 dark:text-ink-200">{drawerRow.agenda}</p></Section>}
            {drawerRow.decisions && <Section title="Keputusan"><p className="text-body whitespace-pre-line text-ink-700 dark:text-ink-200">{drawerRow.decisions}</p></Section>}
            <Section title="Action Item">
              {(drawerRow.action_items ?? []).length === 0 ? <p className="text-caption text-ink-400">Belum ada action item.</p> : (
                <div className="space-y-2">{(drawerRow.action_items ?? []).map((a: any, i: number) => (
                  <div key={i} className="flex items-center justify-between gap-2 p-2.5 rounded-sm border border-ink-200 dark:border-ink-800">
                    <div><p className="text-body text-ink-800 dark:text-ink-100">{a.uraian}</p>
