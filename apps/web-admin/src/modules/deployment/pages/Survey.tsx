...
export default function Survey() {
 ...
 {drawerRow.findings && <Section title="Temuan"><p className="text-body text-ink-700 whitespace-pre-line">{drawerRow.findings}</p></Section>}
 {drawerRow.recommendation && <Section title="Rekomendasi"><p className="text-body text-ink-700 whitespace-pre-line">{drawerRow.recommendation}</p></Section>}
 <Section title="Galeri Foto">
 {gallery.length === 0 ? <EmptyState icon={<ImageIcon size={22} />} title="Belum ada foto" /> : (
 <div className="grid grid-cols-3 gap-2">
 {gallery.map((u, i) => <a key={i} href={u} target="_blank" rel="noreferrer" className="block aspect-square rounded-sm overflow-hidden border border-ink-200">
